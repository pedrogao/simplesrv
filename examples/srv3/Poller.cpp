...
{
    fd_ = epoll_create1(0);
    PanicIf(fd_ == -1, "epoll create error");
...
std::vector<Channel *> Poller::Poll(int timeout)
{
    ...
    PanicIf(nfds == -1, "epoll wait error");
...
void Poller::UpdateChannel(Channel *ch)
{
    ...
    if (!ch->GetExist())
    {
        PanicIf(epoll_ctl(fd_, EPOLL_CTL_ADD, sockfd, &ev) == -1, "epoll add error");
        ch->SetExist();
    }
    else
    {
        PanicIf(epoll_ctl(fd_, EPOLL_CTL_MOD, sockfd, &ev) == -1, "epoll modify error");
    }
}
...
{
    int sockfd = ch->GetSocket()->GetFd();
    PanicIf(epoll_ctl(fd_, EPOLL_CTL_DEL, sockfd, nullptr) == -1, "epoll delete error");
    ch->SetExist(false);
}
...
{
    fd_ = kqueue();
    PanicIf(fd_ == -1, "kqueue create error");
...
void Poller::UpdateChannel(Channel *ch)
{
    ...
    PanicIf(r == -1, "kqueue add event error");
}
...
void Poller::DeleteChannel(Channel *ch)
{
    ...
    int r = kevent(fd_, ev, n, NULL, 0, NULL);
    PanicIf(r == -1, "kqueue delete event error");
}
#endif
